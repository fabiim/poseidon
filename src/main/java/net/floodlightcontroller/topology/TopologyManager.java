...
import static datastore.util.KeySerializationFunctions.LONG_DESERIALIZE;
import static datastore.util.KeySerializationFunctions.LONG_SERIALIZE;
...
//TODO -getAppliedUdaptes will not work on updates from other controllers. 
...
public class TopologyManager implements 
	...
	protected static Logger log = LoggerFactory.getLogger(net.floodlightcontroller.topology.original.TopologyManager.class);
    ...
    //Table : SWITCH_PORT_TOPOLOGY -> Set<Short> 
   //protected Map<Long, Set<Short>> switchPorts;
    protected Table<Long,HashSet<Short>> switchPorts;
    private static String SWITCH_PORTS_TABLE = "SWITCH_PORTS_TABLE_TOPOLOGY_MANAGER";
    
     /**
     * Set of links organized by node port tuple
     */
    // protected Map<NodePortTuple, Set<Link>> switchPortLinks;
    protected Table<NodePortTuple,HashSet<Link>> switchPortLinks; 
    private static String SWITCH_PORTS_LINKS_TABLE = "SWITCH_PORTS_LINKS_TABLE_TOPOLOGY_MANAGER";
   ...
   protected Table<NodePortTuple,HashSet<Link>> directLinks; 
   private static String DIRECT_LINKS_TABLE = "DIRECT_LINKS_TABLE_TOPOLOGY_MANAGER"; 
   ...
   protected Table<NodePortTuple, HashSet<Link>> portBroadcastDomainLinks;
   private static String PORT_BROADCAST_DOMAINS_LINKS_TABLE = "PORT_BROADCAST_DOMAIN_LINKS_TOPOLOGY_MANAGER"; 
   
   /**
    * set of tunnel links
    */
   protected Table<NodePortTuple,NodePortTuple> tunnelPorts; 
   private static String TUNNEL_LINKS_TABLE = "TUNNEL_LINKS_TOPOLOGY_MANAGER"; 
    ...
    protected class UpdateTopologyWorker implements Runnable {
        String currentHash;
        Table<String,String> controllersTable; 
        
        public UpdateTopologyWorker(Table<String,String> controllersTable){
        	this.controllersTable = controllersTable; 
        }
    	@Override 
        public void run() {
            try {
		...
		private void handleMiscellaneousPeriodicEvents() {
			if (!hash(currentInstance).equals(controllersTable.get("currentHash"))){
				createNewInstance();
    ...
    public Set<NodePortTuple> getBroadcastDomainPorts() {
    	return portBroadcastDomainLinks.getAll().keySet();
    }

    @Override
    public Set<NodePortTuple> getTunnelPorts() {
        return tunnelPorts.getAll().keySet();
    }
    ...
    public Route getRoute(long src, short srcPort, long dst, short dstPort, long cookie, 
                          boolean tunnelEnabled) {
        TopologyInstance ti = getCurrentInstance(tunnelEnabled);
        Route route =ti.getRoute(src, srcPort, dst, dstPort,cookie);
        log.info("Get route : src: " + src + " dst: " + dst + "->" + route); 
        return route;  
    }
    ...
    public void init(FloodlightModuleContext context)
        ...
        switchPorts = ds.getTable(SWITCH_PORTS_TABLE,LONG_DESERIALIZE,LONG_SERIALIZE);
        
        switchPortLinks = ds.getTable(SWITCH_PORTS_LINKS_TABLE, NODE_PORT_TUPLE_DESERIALIZE, NODE_PORT_TUPLE_SERIALIZE); 
        directLinks = ds.getTable(DIRECT_LINKS_TABLE,  NODE_PORT_TUPLE_DESERIALIZE,NODE_PORT_TUPLE_SERIALIZE); 
        portBroadcastDomainLinks = ds.getTable(PORT_BROADCAST_DOMAINS_LINKS_TABLE,  NODE_PORT_TUPLE_DESERIALIZE,NODE_PORT_TUPLE_SERIALIZE); 
        tunnelPorts = ds.getTable(TUNNEL_LINKS_TABLE,  NODE_PORT_TUPLE_DESERIALIZE,NODE_PORT_TUPLE_SERIALIZE); 
    ...
    public void startUp(FloodlightModuleContext context) {
        ...
        //ds.clear();
        Table<String,String> controllersTable = ds.getTable(Datastore.CONTROLLERS_SYSTEM_INFO, datastore.util.KeySerializationFunctions.STRING_DESERIALIZE, datastore.util.KeySerializationFunctions.STRING_SERIALIZE); 
        newInstanceTask = new SingletonTask(ses, new UpdateTopologyWorker(controllersTable));
        if (floodlightProvider.getRole() != Role.SLAVE)
            newInstanceTask.reschedule(TOPOLOGY_COMPUTE_INTERVAL_MS,
                                   TimeUnit.MILLISECONDS);
    ...
    public void applyUpdates() {
        ...
        while (ldUpdates.peek() != null) {
            ...
            if (update.getOperation() == UpdateOperation.LINK_UPDATED) {
            	ActivityEvent e = ds.getBenchManager().addActivity(ActivityEvent.addLink());
//            	log.info("LINK UPDATED" + "(" + update.getSrc() + "," + update.getSrcPort() + ")"  + "-> (" + update.getDst() + "," + update.getDstPort() + ")" );
                addOrUpdateLink(update.getSrc(), update.getSrcPort(),
                                update.getDst(), update.getDstPort(),
                                update.getType());
                ds.getBenchManager().endActivity(e);
            } else if (update.getOperation() == UpdateOperation.LINK_REMOVED){
            	ActivityEvent e = ds.getBenchManager().addActivity(ActivityEvent.rmLink());
    //        	log.info("LINK REMOVED" + "(" + update.getSrc() + "," + update.getSrcPort() + ")"  + "-> (" + update.getDst() + "," + update.getDstPort() + ")" ) ;
                removeLink(update.getSrc(), update.getSrcPort(), 
                           update.getDst(), update.getDstPort());
                ds.getBenchManager().endActivity(e);
            } else if (update.getOperation() == UpdateOperation.TUNNEL_PORT_ADDED) {
          	log.info("Tunnel Port added"); 
            	//TODO - tunnels; 
                addTunnelPort(update.getSrc(), update.getSrcPort());
            } else if (update.getOperation() == UpdateOperation.TUNNEL_PORT_REMOVED) {
            	log.info("Tunnel Port removed"); 
            	//TODO - tunnels; 
    ...
    protected boolean createNewInstance() {
    	ActivityEvent e = ds.getBenchManager().addActivity(ActivityEvent.newTopoInstance()); 
          ...
          //TODO - keyset - really! 
          Set<NodePortTuple> nptList = switchPortLinks.getAll().keySet();
          ...
          //TODO - really - values 
          for(NodePortTuple npt: tunnelPorts.getAll().values()) {
              if (switchPortLinks.get(npt) == null) continue;
              for(Link link: switchPortLinks.get(npt)) {
                  removeLinkFromStructure(openflowLinks, link);
              }
          ...
          TopologyInstance nt = new TopologyInstance(switchPorts.getAll(),
                                                     blockedPorts,
                                                     openflowLinks, 
                                                     broadcastDomainPorts,
                                                     //FIXME: not sure if keyset here will really get the core object value. (is se/dese working 100%) 
                                                     tunnelPorts.getAll().keySet());
          
        nt.compute();
        ds.getTable(Datastore.CONTROLLERS_SYSTEM_INFO, datastore.util.KeySerializationFunctions.STRING_DESERIALIZE, datastore.util.KeySerializationFunctions.STRING_SERIALIZE).put("currentHash", hash(nt));
        ds.getBenchManager().endActivity(e); 
        currentInstance = nt;
    ...
    //TODO - this is here just to shut up the copy-paste from createNewInstance. I did not bother to solve the issue of this requirement (removeLinkFromStructure was defined to use a Table directly) 
    /**
    ...
    protected Set<NodePortTuple> identifyBroadcastDomainPorts() {
        ...
        Set<NodePortTuple> broadcastDomainPorts =
                new HashSet<NodePortTuple>();
        broadcastDomainPorts.addAll(this.portBroadcastDomainLinks.getAll().keySet());
        ...
        Map<NodePortTuple, Set<Link>> spLinks =
                new HashMap<NodePortTuple, Set<Link>>();
        for(NodePortTuple npt: switchPortLinks.getAll().keySet()) {
    ...
    private void addPortToSwitch(long s, short p) {
    	...
    	//FIXME: read and addicionar  sucks big time
    	HashSet<Short> shorts =  switchPorts.get(s);
    	if (shorts == null){
    		shorts = new HashSet<Short>();
    		
    	}
    	shorts.add(p);
    	switchPorts.put(s, shorts);
     ...
     * @return
     */
    private boolean addLinkToStructure(Table<NodePortTuple, 
                                       HashSet<Link>> s, Link l) {
        boolean added2n1 = false, added2n2 = false; 

        NodePortTuple n1 = new NodePortTuple(l.getSrc(), l.getSrcPort());
        NodePortTuple n2 = new NodePortTuple(l.getDst(), l.getDstPort());
        
        HashSet<Link> n1Set = s.get(n1);
        HashSet<Link> n2Set = s.get(n2);
        
        if (n1Set == null){
        	n1Set = new HashSet<Link>(); 
        }
        if (s.get(n2) == null) {
        	n2Set = new HashSet<Link>(); 
        }
        
        added2n1 = n1Set.add(l);
        if (added2n1){
        	s.put(n1, n1Set);
        }
        added2n2 = n2Set.add(l);
        if (added2n2){
        	s.put(n2, n2Set);
        }
        
        return added2n1 || added2n2; 
    }
     ...
     * @return
     */
    private boolean removeLinkFromStructure(Table<NodePortTuple, 
                                            HashSet<Link>> table, Link l) {
        boolean rmFromN1 = false, rmFromN2 = false;
        NodePortTuple n1 = new NodePortTuple(l.getSrc(), l.getSrcPort());
        NodePortTuple n2 = new NodePortTuple(l.getDst(), l.getDstPort());

        Map<NodePortTuple, HashSet<Link>> s = table.getAll();
        HashSet<Link> n1Set = s.get(n1);
        HashSet<Link> n2Set = s.get(n2); 
        //TODO check what is faster for smart. contains and then get. or this way. 
        
        if (n1Set != null) {
            rmFromN1 = n1Set.remove(l);
            if (rmFromN1){
            	if (n1Set.isEmpty()){
            		table.remove(n1); 
            	}
    ...
    public void addOrUpdateLink(long srcId, short srcPort, long dstId, 
    	...
    	// CRITICAL 
    	//TODO - not easy ...
    	//FIXME - all over the place. get Set ; update set locally ; put set in table does not work... Updates get lost in the way since other controllers may have been manipulating the same table.
    ...
    private void addOrUpdateTunnelLink(long srcId, short srcPort, long dstId,
			short dstPort) {
	...
	public void removeLink(Link link)  {
        ...
        //TODO - really, can we not handle the fact that there are no links in those switch ports? Sounds kind like you were thinking THAT THESE OPERATIONS ARE FREE!
        long srcNodeId = srcNpt.getNodeId();
		
        if (switchPortLinks.containsKey(srcNpt) == false) {
            if (switchPorts.containsKey(srcNodeId)){
            	HashSet<Short> set =switchPorts.get(srcNodeId);  
            	
            	set.remove(srcNpt.getPortId());
            	switchPorts.put(srcNodeId, set);
            }
              
        }
		
        long dstNodeId = dstNpt.getNodeId();
		if (switchPortLinks.containsKey(dstNpt) == false) {
            if (switchPorts.containsKey(dstNodeId)){
            	HashSet<Short> set = switchPorts.get(dstNodeId);
            	set.remove(dstNpt.getPortId());
                switchPorts.put(dstNodeId, set); 
            }
        }    
        // Remove the node if no ports are present
        if (switchPorts.containsKey(srcNodeId) && 
                switchPorts.get(srcNodeId).isEmpty()) {
            switchPorts.remove(srcNodeId);
        }
        if (switchPorts.containsKey(dstNodeId) && 
                switchPorts.get(dstNodeId).isEmpty()) {
            switchPorts.remove(dstNodeId);
        }
    }
    ...
    */
    ...
    public Map<Long, Set<Short>> getSwitchPorts() {
    	return new HashMap<Long,Set<Short>>(switchPorts.getAll());
    }
    
    public Map<NodePortTuple, Set<Link>> getSwitchPortLinks() {
        return new HashMap<NodePortTuple,Set<Link>>(switchPortLinks.getAll());
    }
    
    public Map<NodePortTuple, Set<Link>> getPortBroadcastDomainLinks() {
        return new HashMap<NodePortTuple,Set<Link>>(portBroadcastDomainLinks.getAll());
    }
